...
# -------------------- 基础路径配置 --------------------
result_dir = "./result_Major revision/LOCO/en/980"
...
# -------------------- ElasticNet网格搜索参数 --------------------
param_grid = {
'alpha': [0.001, 0.1, 0.15, 0.2],  # 正则化强度
'l1_ratio': [0.05, 0.1, 0.15, 0.2, 0.3, 0.5],  # L1/L2混合比例，0=Ridge, 1=Lasso
'max_iter': [1000, 5000],  # 最大迭代次数
'selection': ['cyclic', 'random']  # 坐标下降方式
}
...
def evaluate_model(model, X_train, y_train, X_test, y_test):
    ...
    n_total_coef = len(model.coef_)
    sparsity = n_nonzero_coef / n_total_coef * 100
    l1_ratio = model.l1_ratio

    # ElasticNet类型判断
    if l1_ratio == 0:
        net_type = "Ridge回归 (L2正则化)"
    elif l1_ratio == 1:
        net_type = "Lasso回归 (L1正则化)"
    else:
        net_type = f"ElasticNet (L1/L2混合: {l1_ratio:.1%})"

    print("\n" + "=" * 70)
    print("ElasticNet模型性能评估结果")
    print("=" * 70)
    ...
    print("=" * 70)
    print(f"\nElasticNet模型信息:")
    print(f"  正则化类型: {net_type}")
    print(f"  alpha值: {model.alpha}")
    print(f"  非零系数数量: {n_nonzero_coef} / {n_total_coef}")
...
def save_results(model, results, feature_columns, target_column, train_names=None, test_names=None):
    ...
    with open(params_path, 'w', encoding='utf-8') as f:
        ...
        if hasattr(model, 'coef_'):
            ...
            if len(nonzero_indices) > 0:
                for i, idx in enumerate(nonzero_indices, 1):
                    feature = feature_columns[idx]
                    coef = model.coef_[idx]
                    f.write(f"  {i:3d}. {feature:<30}: {coef:.6f}\n")
            else:
                f.write("  所有系数都为0\n")

            f.write(f"\n稀疏性统计:\n")
            f.write(f"  总特征数: {len(model.coef_)}\n")
            f.write(f"  非零系数数: {results['n_nonzero_coef']}\n")
            f.write(f"  稀疏度: {results['sparsity']:.2f}%\n")
    ...
    metrics_df = pd.DataFrame({
        ...
        'L1_Ratio': [results['l1_ratio'], results['l1_ratio']],
        'Alpha': [results['alpha'], results['alpha']],
        'Sparsity(%)': [results['sparsity'], results['sparsity']],
        'Nonzero_Features': [results['n_nonzero_coef'], results['n_nonzero_coef']]
    })
...
def create_visualizations(results, model, X_train, X_test, feature_columns, target_column):
    ...
    fig.suptitle(f'ElasticNet Regression 模型性能可视化\n目标变量: {target_column}\n模型类型: {results["net_type"]}',
                 fontsize=16, y=1.02)
    ...
    if hasattr(model, 'coef_'):
        nonzero_indices = np.where(model.coef_ != 0)[0]
        nonzero_features = [feature_columns[i] for i in nonzero_indices]
        nonzero_coefs = model.coef_[nonzero_indices]

        if len(nonzero_features) > 0:
            fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(16, 12))

            sorted_indices = np.argsort(np.abs(nonzero_coefs))
            sorted_features = [nonzero_features[i] for i in sorted_indices]
            sorted_coefs = [nonzero_coefs[i] for i in sorted_indices]

            colors_bar = ['red' if x < 0 else 'blue' for x in sorted_coefs]
            y_pos = np.arange(len(sorted_features))

            bars = ax1.barh(y_pos, sorted_coefs, color=colors_bar, alpha=0.7)
            ax1.set_yticks(y_pos)
            ax1.set_yticklabels(sorted_features, fontsize=9)
            ax1.set_xlabel('系数值', fontsize=12)
            ax1.set_title(f'ElasticNet非零特征系数 (共{len(nonzero_features)}个, L1比例={results["l1_ratio"]:.2f})',
            ...
            ax2.pie([results['sparsity'], 100 - results['sparsity']],
                    labels=['非零系数', '零系数'],
            ...
            ax2.text(0, 0.2, f'L1比例: {results["l1_ratio"]:.2f}',
                     ha='center', va='center', fontsize=12, fontweight='bold')
            ax2.text(0, -0.2, f'Alpha: {results["alpha"]}',
                     ha='center', va='center', fontsize=11)

            ax2.set_title('ElasticNet模型稀疏性与正则化参数', fontsize=14, fontweight='bold')
    ...
    ax.set_title('ElasticNet模型性能指标对比', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ax.set_xticklabels(datasets_plot, fontsize=11)
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3, axis='y')

    model_info = f"Alpha: {results['alpha']} | L1 Ratio: {results['l1_ratio']:.2f} | 稀疏度: {results['sparsity']:.1f}%"
    ...
    regularization_info = f"""
    ElasticNet 正则化参数:

    Alpha (正则化强度): {results['alpha']}
    L1 Ratio (L1比例): {results['l1_ratio']:.3f}

    模型特性:
    - 当 L1 Ratio = 0: 等同于 Ridge 回归 (纯L2正则化)
    - 当 L1 Ratio = 1: 等同于 Lasso 回归 (纯L1正则化)
    - 当 0 < L1 Ratio < 1: ElasticNet (L1/L2混合)

    当前模型:
    {results['net_type']}

    稀疏性:
    - 总特征数: {results['n_total_coef']}
    - 非零系数: {results['n_nonzero_coef']}
    - 稀疏度: {results['sparsity']:.2f}%

    测试集性能:
    - R²: {results['test']['r2']:.4f}
    - MSE: {results['test']['mse']:.4f}
    - RMSE: {results['test']['rmse']:.4f}
    - MAE: {results['test']['mae']:.4f}
    - 过拟合程度: {results['overfitting_score']:.4f}
    """

    ax.text(0.1, 0.5, regularization_info, transform=ax.transAxes,
            fontsize=11, verticalalignment='center',
            bbox=dict(boxstyle='round', facecolor='lightblue', alpha=0.8))

    ax.set_title('ElasticNet正则化参数与模型特性', fontsize=14, fontweight='bold')
    ax.axis('off')

    reg_path = os.path.join(result_dir_path, "regularization_info.png")
    plt.savefig(reg_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"正则化信息图已保存到: {reg_path}")

    # 6. ElasticNet算法分析图
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 6))

    regularization_types = ['纯L2\n(Ridge)', 'L1/L2混合\n(ElasticNet)', '纯L1\n(Lasso)']
    bias = [1.0, 0.8, 0.6]
    variance = [0.7, 0.9, 1.0]
    sparsity_scores = [0.2, 0.5, 0.9]

    x = np.arange(len(regularization_types))
    width = 0.25

    bars1_plot = ax1.bar(x - width, bias, width, label='偏差', color='lightblue', alpha=0.8)
    bars2_plot = ax1.bar(x, variance, width, label='方差', color='lightgreen', alpha=0.8)
    bars3_plot = ax1.bar(x + width, sparsity_scores, width, label='稀疏性', color='lightcoral', alpha=0.8)

    ax1.set_xlabel('正则化类型', fontsize=12)
    ax1.set_ylabel('相对性能', fontsize=12)
    ax1.set_title('L1/L2正则化效果对比', fontsize=14, fontweight='bold')
    ax1.set_xticks(x)
    ax1.set_xticklabels(regularization_types, fontsize=11)
    ax1.legend(fontsize=10)
    ax1.grid(True, alpha=0.3, axis='y')

    sparsity_levels = ['低稀疏性\n(所有特征)', '中等稀疏性\n(部分特征)', '高稀疏性\n(少数特征)']
    interpretability = [0.6, 0.8, 1.0]
    generalization = [0.7, 0.9, 0.8]

    ax2.plot(sparsity_levels, interpretability, 'o-', linewidth=2, markersize=8, label='可解释性', color='blue')
    ax2.plot(sparsity_levels, generalization, 's-', linewidth=2, markersize=8, label='泛化能力', color='green')

    current_sparsity = results['sparsity'] / 100
    if current_sparsity < 0.3:
        current_level_idx = 0
    elif current_sparsity < 0.7:
        current_level_idx = 1
    else:
        current_level_idx = 2

    ax2.plot(sparsity_levels[current_level_idx], interpretability[current_level_idx],
             'ro', markersize=12, label=f'当前模型\n({results["sparsity"]:.1f}%)')
    ax2.plot(sparsity_levels[current_level_idx], generalization[current_level_idx],
             'ro', markersize=12)

    ax2.set_xlabel('稀疏性水平', fontsize=12)
    ax2.set_ylabel('性能评分', fontsize=12)
    ax2.set_title('稀疏性对模型性能的影响', fontsize=14, fontweight='bold')
    ax2.legend(fontsize=10)
    ax2.grid(True, alpha=0.3)

    plt.tight_layout()
    algorithm_path = os.path.join(result_dir_path, "elasticnet_algorithm_analysis.png")
    plt.savefig(algorithm_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"ElasticNet算法分析图已保存到: {algorithm_path}")
...
def main():
    ...
    print("开始 ElasticNet Regression 建模流程")
    print("=" * 70)
    print("ElasticNet是一种结合L1和L2正则化的线性回归模型")
    print("特点：可以自动进行特征选择，平衡偏差和方差")
    print("应用场景：高维数据、多重共线性、特征选择")
    print("=" * 70)
    ...
    # 5. 保存结果
    metrics_df, coef_df = save_results(best_model, results, feature_columns, target_column, train_names, test_names)
    ...
    # 7. 打印总结
    print("\n" + "=" * 70)
    print("ElasticNet建模流程完成!")
    print("=" * 70)
    print(f"最佳模型参数:")
    print(f"  alpha: {best_model.alpha}")
    print(f"  l1_ratio: {best_model.l1_ratio}")
    print(f"  selection: {best_model.selection}")
    ...
    print(f"\n模型稀疏性:")
    print(f"  非零系数: {results['n_nonzero_coef']} / {results['n_total_coef']}")
    print(f"  稀疏度: {results['sparsity']:.2f}%")
    print(f"\n过拟合分析:")
    ...
    for i in range(min(5, len(coef_df_sorted))):
        feature = coef_df_sorted.iloc[i]['Feature']
        coef = coef_df_sorted.iloc[i]['Coefficient']
        abs_coef = coef_df_sorted.iloc[i]['Abs_Coefficient']
        print(f"  {i + 1}. {feature}: {coef:.6f} (绝对值: {abs_coef:.6f})")
