...
# -------------------- 基础路径配置 --------------------
result_dir = "./result_Major revision/LOCO/svm/777"
...
# -------------------- SVM网格搜索参数 --------------------
param_grid = {
    'svm__kernel': ['linear', 'poly','rbf' ],  # 核函数类型
    'svm__C': [0.1, 1, 10, 100],  # 正则化参数
    'svm__epsilon': [0.01, 0.1, 0.2],  # epsilon-insensitive loss的参数
    'svm__gamma': ['scale', 'auto', 0.01, 0.1, 1],  # RBF核的gamma参数
    'svm__degree': [2, 3, 4],  # 多项式核的阶数（仅在kernel='poly'时使用）
    'svm__coef0': [0.0, 0.5, 1.0],  # 核函数中的独立项
}
...
def perform_grid_search(X_train, y_train, train_names=None, feature_columns=None, target_column=None):
    ...
    print("\n正在执行SVM网格搜索...")
    print("SVM（支持向量机）是一种基于核方法的强大回归模型")
...
def evaluate_model(model, X_train, y_train, X_test, y_test, feature_columns):
    ...
    # 获取支持向量数量（SVM特有）
    if hasattr(model.named_steps['svm'], 'support_vectors_'):
        n_support_vectors = len(model.named_steps['svm'].support_vectors_)
    else:
        n_support_vectors = 0

    # 获取模型参数
    svm_params = model.named_steps['svm'].get_params()
    kernel_type = svm_params['kernel']

    # 计算特征重要性（仅适用于线性核）
    if kernel_type == 'linear':
        feature_importance = np.abs(model.named_steps['svm'].coef_[0])
    else:
        # 对于非线性核，使用均匀分布作为占位符
    ...
    print("=" * 70)
    print(f"\nSVM模型信息:")
    print(f"  核函数类型: {kernel_type}")
    print(f"  正则化参数C: {svm_params['C']}")
    print(f"  epsilon参数: {svm_params['epsilon']}")
...
def save_results(model, results, feature_columns, target_column, train_names=None, test_names=None):
    ...
    with open(params_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f"  支持向量数量: {results['n_support_vectors']}\n")
        f.write(f"  核函数类型: {results['kernel_type']}\n")
        ...
        f.write(f"\nSVM模型特点:\n")
        f.write(f"  - 基于核方法的回归模型\n")
        f.write(f"  - 在高维空间中寻找最优超平面\n")
        f.write(f"  - 使用epsilon-insensitive损失函数\n")
        f.write(f"  - 对异常值相对鲁棒\n")
        f.write(f"  - 需要特征标准化\n")
        f.write(f"  - 计算复杂度与样本数相关\n\n")

        if results['kernel_type'] == 'linear':
            f.write("特征重要性 (前20个):\n")
            ...
            f.write("注意: 非线性核函数的特征重要性需要使用SHAP值分析\n")
    ...
    metrics_df = pd.DataFrame({
        ...
        'Support_Vectors': [results['n_support_vectors'], results['n_support_vectors']],
        'Kernel_Type': [results['kernel_type'], results['kernel_type']],
...
def create_visualizations(results, model, X_train, X_test, feature_columns, target_column):
    ...
    fig.suptitle(
        f'SVM Regression 模型性能可视化\n目标变量: {target_column}\n核函数={results["kernel_type"]}, C={results["model_params"]["C"]}',
        fontsize=16, y=1.02)
    ...
    # 3. 特征重要性图（仅适用于线性核）
    if results['kernel_type'] == 'linear' and results['feature_importance'] is not None:
        ...
        ax1.set_xlabel('重要性分数 (系数的绝对值)', fontsize=12)
        ax1.set_title(f'特征重要性 (SVM线性核) - 显示{max_display_features}个特征', fontsize=14, fontweight='bold')
        ...
        ax2.set_title('累积特征重要性 (SVM线性核)', fontsize=14, fontweight='bold')
    ...
    ax.set_title('SVM模型性能指标对比', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ax.set_xticklabels(datasets_plot, fontsize=11)
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3, axis='y')

    model_info = f"核函数: {results['kernel_type']} | C: {results['model_params']['C']} | ε: {results['model_params']['epsilon']}"
    ...
    ax.text(0.5, 0.7, f'核函数类型: {results["kernel_type"]}',
            transform=ax.transAxes, fontsize=14, ha='center')
    ax.text(0.5, 0.6, f'支持向量数量: {results["n_support_vectors"]}',
            transform=ax.transAxes, fontsize=14, ha='center')
    ax.text(0.5, 0.5, f'正则化参数C: {results["model_params"]["C"]}',
            transform=ax.transAxes, fontsize=14, ha='center')
    ax.text(0.5, 0.4, f'epsilon参数: {results["model_params"]["epsilon"]}',
            transform=ax.transAxes, fontsize=14, ha='center')
    ...
    ax.set_title('SVM模型网络结构总结', fontsize=16)
    ax.axis('off')
    ...
    if save_shap_results:
        try:
            ...
            # 创建一个包装器函数来处理Pipeline的预测
            def pipeline_predict(X):
                return model.predict(X)

            # 对于SVM，使用KernelExplainer
            background_sample_size = min(50, X_train.shape[0])
            if isinstance(X_train, pd.DataFrame):
                background_data = X_train.iloc[:background_sample_size]
            else:
                background_data = X_train[:background_sample_size]

            explainer = shap.KernelExplainer(pipeline_predict, background_data)
            ...
            print(f"计算SHAP值，样本大小: {sample_size}...")
            ...
            shap.summary_plot(shap_values, X_sample, plot_type="bar",
                             ...
                             show=False)
            plt.title(f'SHAP特征重要性 (SVM {results["kernel_type"]}核) - 所有{len(feature_columns)}个特征',
            ...
            shap.summary_plot(shap_values, X_sample,
                             max_display=len(feature_columns),
                             show=False)
            plt.title(f'SHAP summary plot (SVM {results["kernel_type"]}核) - all{len(feature_columns)}个特征',
                      fontsize=14, fontweight='bold')
            plt.tight_layout()
            ...
            print("跳过SHAP分析，SHAP对SVM的Pipeline支持有限")

    # 7. SVM算法分析图
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 6))

    # 图1：支持向量分布
    if results['n_support_vectors'] > 0:
        n_total_samples = len(X_train)

        categories = ['支持向量', '非支持向量']
        counts = [results['n_support_vectors'], n_total_samples - results['n_support_vectors']]
        colors_pie = ['lightcoral', 'lightblue']

        ax1.pie(counts, labels=categories, autopct='%1.1f%%', colors=colors_pie,
                startangle=90, explode=(0.1, 0))
        ax1.set_title(f'支持向量分布\n(占总样本的{results["n_support_vectors"] / n_total_samples * 100:.1f}%)',
                      fontsize=14, fontweight='bold')
    else:
        ax1.text(0.5, 0.5, '支持向量信息不可用\n(可能使用线性核)',
                 ha='center', va='center', transform=ax1.transAxes, fontsize=12)
        ax1.set_title('支持向量分布', fontsize=14, fontweight='bold')

    # 图2：不同核函数性能对比
    kernels = ['线性核', 'RBF核', '多项式核']
    training_speed = [1.0, 0.7, 0.5]
    interpretability = [1.0, 0.6, 0.5]
    flexibility = [0.6, 1.0, 0.9]

    x = np.arange(len(kernels))
    width = 0.25

    bars1 = ax2.bar(x - width, training_speed, width, label='训练速度', color='lightgreen', alpha=0.8)
    bars2 = ax2.bar(x, interpretability, width, label='可解释性', color='lightblue', alpha=0.8)
    bars3 = ax2.bar(x + width, flexibility, width, label='灵活性', color='lightcoral', alpha=0.8)

    ax2.set_xlabel('核函数类型', fontsize=12)
    ax2.set_ylabel('相对性能', fontsize=12)
    ax2.set_title('不同SVM核函数性能对比', fontsize=14, fontweight='bold')
    ax2.set_xticks(x)
    ax2.set_xticklabels(kernels, fontsize=11)
    ax2.legend(fontsize=10)
    ax2.grid(True, alpha=0.3, axis='y')

    kernel_mapping = {'linear': 0, 'rbf': 1, 'poly': 2}
    if results['kernel_type'] in kernel_mapping:
        idx = kernel_mapping[results['kernel_type']]
        ax2.text(idx, -0.15, '当前使用', ha='center', va='top',
                 fontsize=11, fontweight='bold', color='red')

    plt.tight_layout()
    svm_analysis_path = os.path.join(result_dir_path, "svm_algorithm_analysis.png")
    plt.savefig(svm_analysis_path, dpi=300, bbox_inches='tight')
    plt.close()

    print(f"SVM算法分析图已保存到: {svm_analysis_path}")
...
def main():
    ...
    print("开始 SVM Regression 建模流程")
    print("=" * 70)
    print("SVM（支持向量机）是一种基于核方法的强大回归模型")
    print("特点：在高维空间寻找最优超平面，对异常值相对鲁棒")
    print("支持多种核函数：线性、RBF、多项式等")
    print("=" * 70)
    ...
    print(f"目标变量范围: [{y_train.min():.4f}, {y_train.max():.4f}]")
    print(f"目标变量均值: {y_train.mean():.4f}, 标准差: {y_train.std():.4f}")
    ...
    print("=" * 70)
    print(f"最佳模型参数:")
    print(f"  核函数类型: {results['kernel_type']}")
    print(f"  正则化参数C: {results['model_params']['C']}")
    print(f"  epsilon参数: {results['model_params']['epsilon']}")
    print(f"  支持向量数量: {results['n_support_vectors']}")

    if results['kernel_type'] == 'rbf':
        print(f"  gamma参数: {results['model_params']['gamma']}")
    elif results['kernel_type'] == 'poly':
        print(f"  多项式阶数: {results['model_params']['degree']}")
    ...
    print(f"\n模型诊断信息:")
    print(f"  支持向量占比: {results['n_support_vectors'] / len(X_train) * 100:.1f}%")
    ...
    if results['overfitting_score'] > 0.1:
        ...
        print(f"    1. 减小C参数值（增加正则化强度）")
        print(f"    2. 增大epsilon参数值")
        print(f"    3. 使用更简单的核函数（如线性核）")
    ...
    # 显示最重要的5个特征（如果可用）
    if results['kernel_type'] == 'linear' and 'importance_df' in locals():
        print(f"\n最重要的5个特征:")
    ...
    elif results['kernel_type'] != 'linear':
        print(f"\n注意：{results['kernel_type']}核函数的特征重要性需要使用SHAP值分析")


if __name__ == "__main__":
    try:
        from sklearn.svm import SVR
        print("✓ scikit-learn库已安装（包含SVM）")
    except ImportError:
        print("✗ 错误：需要安装scikit-learn库，请运行: pip install scikit-learn")
        exit(1)
